...
public class MyController implements Initializable {
    ...
    private  boolean isAllowedToInsert= true;// help avoid more than one disk
    public void createPlayground(){
        Platform.runLater(() -> setNamesBtn.requestFocus());
        ...
        for (int col = 0; col < COLUMNS; col++) {
            ...
            rectangle.setOnMouseClicked(mouseEvent -> {
                if (isAllowedToInsert) {
                    isAllowedToInsert=false;//prevents adding more than one disk
    ...
    private void insertDisc(Disc disc, int column) {
        ...
        if(row<0)//column is full
            return;

        insertedDiscsArray[row][column]= disc ;// for developer only
        ...
        translateTransition.setOnFinished(actionEvent -> {
            isAllowedToInsert=true;//allows next player to insert the disk
